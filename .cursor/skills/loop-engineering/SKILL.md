...
オープンな実装 PR を見る。[references/pr-review.md](references/pr-review.md)

- head より新しい `[Major]` / `[Minor]` がある → **この実行ではその PR の指摘対応だけ**をする（同じ実行で別 issue は実装しない）
- レビューコメントがまだ無い → **次のバックログ issue を止めない。** この実行で今出した PR なら最大約 10 分待ってよい。来なければレビュー待ちのまま終了してよい。次のオートメーションは未マージを理由に止まらず、次 issue に進む
- 指摘を直したあとも、その PR のフォローアップレビューを最大 1 ラウンド待つ（その実行の対象 PR に限る）
- **未マージの実装 PR が残っていても、未対応の Major/Minor が無ければ次のバックログ issue を始める**

## 1. issue 選定

`BACKLOG.md` の **未完了の先頭** を次の着手にする。未マージ PR があるだけでは選定しない理由にしない。未対応の `[Major]` / `[Minor]` があるときだけ、この実行は指摘対応に充てて選定しない。
...
着手不能（本文が空、キャンセルと現行方針が矛盾）なら、ユーザー判断が要るか技術リトライかを §5 / §6 で分岐する。依存先が未マージ PR なだけなら着手不能ではない。§3 どおりそのブランチから切る。
...
- ブランチ名は `feature/<short-kebab>-<短いランダム>`。接尾辞は実行ごとに変える（例: `openssl rand -hex 2`）。**特定の PR の接尾辞を固定で使い回さない**
- 実行環境がブランチ名のパターンを別途指定しているときだけ、その指定を優先する
- **切り元（base）は依存で決める。未マージ PR があるから止まらない:**
  - 着手 issue が、未マージ PR の成果物（その issue のコード）を必要とする → **その PR の head ブランチから** 切る。新しい PR の base もその head ブランチにする（スタック）
  - 着手 issue が default branch の内容だけで完結する → **default branch（通常 `main`）から** 切る。未マージ PR のコミットを混ぜない
  - 依存が複数の未マージ PR にまたがる → 必要な変更がすべて乗っている **スタックの先端** から切る
  - 依存するかは issue 本文・受け入れ条件と、default branch に無いファイル／API で判断する。バックログの並びが先というだけでは依存ではない（例: ドメインモデル #3 は GitHub Pages #12 に依存しない）
...
4. 再レビューを最大 1 ラウンド待つ
5. レビューがこの実行中に来なければ、この実行はここで終了してよい。次のオートメーションは §0.4 のとおり、未対応指摘が無ければ次のバックログ issue に進む
...
- レビュー待ちのまま時間切れ。次実行は未対応指摘が無ければ次 issue に進む旨を報告した
...
- 未対応の `[Major]` / `[Minor]` を残したまま、同じ実行で別 issue に進む
- 未マージ PR や「レビュー未着」だけを理由に次の issue を始めない
- 独立した issue を未マージ PR のブランチから切って無関係な差分を乗せる
- 未マージの成果物が必要なのに default branch から切って壊れた実装にする
- 判断待ちなのに「仮決め」で実装を進める
